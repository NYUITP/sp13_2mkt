...
public class CompanyService {
	...
	public List<Company> getAll() {
		...
		DBCollection coll = MongoDBFactory.getCollection(
				CommonStrings.DATABASENAME.getLabel().toString(),
		...
		while (cur.hasNext()) {
			...
			company.setId(Integer.valueOf(dbObject.get(
					CompanyEnum._ID.getLabel()).toString()));
			company.setName(dbObject.get(CompanyEnum.NAME.getLabel())
					.toString());
			company.setFollower_count(Integer.valueOf(dbObject.get(
					CompanyEnum.FOLLOWER_COUNT.getLabel()).toString()));
			double total_funds_in_thousands = Double.valueOf(dbObject.get(
					CompanyEnum.TOTAL_FUNDING.getLabel()).toString());
			double total_funds_in_millions = (total_funds_in_thousands / 1000.0);
			company.setTotal_funding(Double.valueOf(Double.valueOf(String
					.format("%.2f", total_funds_in_millions))));
			company.setProduct_desc(dbObject.get(
					CompanyEnum.PRODUCT_DESC.getLabel()).toString());
			company.setLogo_url(dbObject.get(CompanyEnum.LOGO_URL.getLabel())
					.toString());
			company.setCompany_url(dbObject.get(
					CompanyEnum.COMPANY_URL.getLabel()).toString());
			company.setTwitter_url(dbObject.get(
					CompanyEnum.TWITTER_URL.getLabel()).toString());
			company.setBlog_url(dbObject.get(CompanyEnum.BLOG_URL.getLabel())
					.toString());
			company.setHigh_concept(dbObject.get(
					CompanyEnum.HIGH_CONCEPT.getLabel()).toString());
			company.setAngellist_url(dbObject.get(
					CompanyEnum.ANGLELIST_URL.getLabel()).toString());
	...
	public Company get(String id) {
		...
		company.setId(Integer.valueOf(dbObject.get(CompanyEnum._ID.getLabel())
				.toString()));
		...
		double total_funds_in_thousands = Double.valueOf(dbObject.get(
				CompanyEnum.TOTAL_FUNDING.getLabel()).toString());
		double total_funds_in_millions = (total_funds_in_thousands / 1000.0);
		company.setTotal_funding(Double.valueOf(Double.valueOf(String.format(
				"%.2f", total_funds_in_millions))));
		company.setProduct_desc(dbObject.get(
				CompanyEnum.PRODUCT_DESC.getLabel()).toString());
		company.setLogo_url(dbObject.get(CompanyEnum.LOGO_URL.getLabel())
				.toString());
		company.setCompany_url(dbObject.get(CompanyEnum.COMPANY_URL.getLabel())
				.toString());
		company.setTwitter_url(dbObject.get(CompanyEnum.TWITTER_URL.getLabel())
				.toString());
		company.setBlog_url(dbObject.get(CompanyEnum.BLOG_URL.getLabel())
				.toString());
		company.setHigh_concept(dbObject.get(
	...
	public List<Company> filterByFunds(String[] fundRange) {
		...
		while (cur.hasNext()) {
			...
			company.setFollower_count(Integer.valueOf(dbObject.get(
					CompanyEnum.FOLLOWER_COUNT.getLabel()).toString()));
			company.setTotal_funding(Double.valueOf(dbObject.get(
					CompanyEnum.TOTAL_FUNDING.getLabel()).toString()));
			company.setProduct_desc(dbObject.get(
					CompanyEnum.PRODUCT_DESC.getLabel()).toString());
			company.setLogo_url(dbObject.get(CompanyEnum.LOGO_URL.getLabel())
			...
			for (String each : fundRange) {
				...
				if (each.equals("1")) {
					...
					if (company.getTotal_funding() < 1000000
							&& company.getTotal_funding() >= 500000)
						items.add(company);
